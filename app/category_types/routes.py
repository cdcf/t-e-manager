...
def edit_category_type(id):
    ...
    form = EditCategoryTypeForm(category_type.name)
    if current_user.can(Permission.WRITE) and form.validate_on_submit():
        category_type.name = form.name.data
        category_type.icon = form.icon.data
        category_type.category_id = form.category_id.data.id
        db.session.add(category_type)
        db.session.commit()
        flash('Your changes have been saved.', 'success')
        return redirect(url_for('category_types.edit_category_type', id=id))
    elif request.method == 'GET':
        form.category_id.data = category_type.category_category_type
    ...
    return render_template('settings/edit_category_type.html', title='Edit Category Type', form=form,
                           category_types=category_types, pagination=pagination, id=id)
